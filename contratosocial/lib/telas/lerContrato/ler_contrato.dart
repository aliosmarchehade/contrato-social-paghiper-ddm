    ...
    await db.transaction((txn) async {
      try {
        ...
        for (var socio in MockData.mockSocios) {
          ...
          await DAOSocio().salvar(newSocio, db: txn);
        }
        ...
        for (var clausula in MockData.mockClausulas) {
          ...
          await DAOClausulas().salvar(newClausula, db: txn);
        }
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Erro ao inserir mock: $e")),
        );
        rethrow;
      }
    });
                      ...
                      if (_fileName != null) ...[
                        ...
                        Container(
                          ...
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              const Icon(Icons.picture_as_pdf, color: Color(0xFF0860DB)),
                              const SizedBox(width: 8),
                              Flexible(
                                child: Text(
                                  _fileName!,
                              ...
                              const SizedBox(width: 8),
                              IconButton(
                                icon: const Icon(Icons.visibility, color: Color(0xFF0860DB)),
                                onPressed: _viewPdf,
                                tooltip: 'Visualizar PDF',
                              ),
                            ],
